...
import { motion, useMotionValue, useSpring, useTransform, AnimatePresence, animate, MotionValue } from "framer-motion";
...
const NavPoint: React.FC<NavPointProps> = ({ angle, radiusSpring, label, detail, to, index, counterRotation, onHover }) => {
  ...
  const getAnimationProps = () => {
    switch (index) {
      case 0: return { initial: { opacity: 0, y: -20 }, animate: { opacity: 1, y: 0 }, exit: { opacity: 0, y: -20 } };
      case 1: return { initial: { opacity: 0, x: 20 }, animate: { opacity: 1, x: 0 }, exit: { opacity: 0, x: 20 } };
      case 2: return { initial: { opacity: 0, x: -20 }, animate: { opacity: 1, x: 0 }, exit: { opacity: 0, x: -20 } };
      default: return { initial: { opacity: 0, scale: 0.95 }, animate: { opacity: 1, scale: 1 }, exit: { opacity: 0, scale: 0.95 } };
    }
  };
  ...
  return (
    <motion.div
      ...
      onMouseEnter={() => { setIsLocalHover(true); onHover(index); }}
      onMouseLeave={() => { setIsLocalHover(false); onHover(null); }}
    >
      <Link to={to}>
        <div className="relative flex items-center justify-center w-24 h-24 group">
          <motion.div
            ...
            animate={{
              ...
              boxShadow: isLocalHover ? "0 0 30px rgba(212, 175, 55, 0.6)" : "0 0 0px rgba(212, 175, 55, 0)"
            }}
          />
...
export const NebulaGraphic = () => {
  ...
  useEffect(() => {
    ...
    return () => { if (rotationControls.current) rotationControls.current.stop(); };
      ...
      >
        ...
        >
          <svg viewBox="0 0 200 200" className="absolute inset-0 w-full h-full overflow-visible">
            <circle cx="100" cy="100" r="95" fill="none" stroke="#d4af37" strokeWidth="0.1" strokeDasharray="3 3" className="opacity-40" />
            <line x1="100" y1="5" x2="100" y2="195" stroke="#ffffff" strokeWidth="0.05" className="opacity-20" />
            <line x1="5" y1="100" x2="195" y2="100" stroke="#ffffff" strokeWidth="0.05" className="opacity-20" />

            <circle cx="100" cy="100" r="65" fill="none" stroke="#ffffff" strokeWidth="0.15" className="opacity-20" />
          ...
          <div className="lg:hidden absolute -bottom-10 left-1/2 -translate-x-1/2 flex items-center gap-5 whitespace-nowrap z-20">
            {[
              ...
              <Link key={i} to={link.to} className="font-mono text-[9px] uppercase tracking-[0.35em] text-white/50 hover:text-white transition-colors">
                {link.label}
              </Link>
            ))}
          </div>
