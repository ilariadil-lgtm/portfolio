...
export const ProjectNavigation: React.FC<ProjectNavigationProps> = ({ prev, next }) => {
    ...
    <section className="border-t border-primary/10 px-8 md:px-16 lg:px-24 py-16 bg-[#f5f2ed]">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8">
        ...
        <div className="flex-1 flex justify-start">
          {prev && (
            <Link to={prev.url} className="group flex flex-col items-start text-left">
              <span className="font-typewriter text-[9px] uppercase tracking-[0.4em] text-primary/60 mb-2">{t('project_nav.prev')}</span>
              <span className="font-display text-2xl md:text-3xl text-[#3d0f1a] group-hover:text-primary transition-colors flex items-center gap-3">
        ...
        <div className="shrink-0 hidden md:block">
          ...
          >
            ...
            <Grid size={18} className="text-[#3d0f1a] group-hover:text-white relative z-10 transition-colors" />
          </Link>
        </div>
        ...
        <div className="flex-1 flex justify-end text-right">
          {next && (
            <Link to={next.url} className="group flex flex-col items-end">
              <span className="font-typewriter text-[9px] uppercase tracking-[0.4em] text-primary/60 mb-2">{t('project_nav.next')}</span>
              <span className="font-display text-2xl md:text-3xl text-[#3d0f1a] group-hover:text-primary transition-colors flex items-center gap-3">
                {next.title}
