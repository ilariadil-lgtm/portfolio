      ...
      <section className="pt-32 md:pt-48 pb-16 px-8 md:px-16 lg:px-24">
        ...
        <Reveal delay={200}>
          <p className="font-body text-muted-foreground leading-relaxed mt-8 max-w-xl">
            Un viaggio che inizia tra i corridoi dell'Accademia di Belle Arti e arriva 
            alle architetture cloud. La stessa ossessione per i dettagli, applicata a 
          ...
          {techStack.map((tech, i) => (
            <Reveal key={tech.name} delay={i * 100}>
              <div className="flex flex-col items-center gap-4 p-8 border border-border rounded-lg hover:border-primary/30 transition-colors">
                ...
                <span className="font-body text-xs uppercase tracking-[0.2em] text-muted-foreground">{tech.name}</span>
              </div>
            </Reveal>
          ))}
        </div>
